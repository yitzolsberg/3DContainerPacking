using ContainerPacking2.Algorithms;
		...
		public static List<ContainerPackingResult> Pack(List<Container> containers, List<Item> itemsToPack, List<int> algorithmTypeIDs)
		{
			...
			Parallel.ForEach(containers, container =>
			{
				...
				Parallel.ForEach(algorithmTypeIDs, algorithmTypeID =>
				{
					...
					double containerVolume = (double)container.Length * container.Width * container.Height;
                    double itemVolumePacked = algorithmResult.PackedItems.Sum(i => (double)i.Volume);
                    double itemVolumeUnpacked = algorithmResult.UnpackedItems.Sum(i => (double)i.Volume);
